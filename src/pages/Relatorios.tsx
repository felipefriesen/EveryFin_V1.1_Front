...
export default function Relatorios() {
  const [type, setType] = useState<'entrada' | 'saida' | 'geral'>('geral')
  ...
  return (
    <div>
      ...
      <div className="flex space-x-4 mb-6">
        ...
        <Select label="Tipo" options={[{label: 'Todas', value: 'geral'}, {label: 'Entradas', value: 'entrada'}, {label: 'Saídas', value: 'saida'}]} value={type} onChange={e => setType(e.target.value as any)} />
        <Button onClick={generate}>Gerar</Button>
      </div>
      <table className="min-w-full bg-white">
        <thead>
          <tr><th>ID</th><th>Tipo</th><th>Descrição</th><th>Valor</th><th>Data</th><th>Categoria</th><th>Cliente</th></tr>
        </thead>
        <tbody>
          {data.map(tx => (
            <tr key={tx.id}>
